...
const UserFilterRaffle = (props: any) => {
  ...
  const { item, id, url, walletBalance, isFilterRaffles, isFilterByItem, setFilterByItem, setLoading, buyTicketNFT } =
    props;
    ...
    <div className="basis-[32%] mt-6" key={item.id}>
      <div className="rounded-[0.9rem] overflow-hidden border-4 border-[#606060]">
        <div className="relative">
          <img src={item?.image} alt="BADImage" className="md:min-h-[300px] w-full object-cover" />
          ...
          <div className="pt-2 pl-3 pr-3">
            <div className="flex justify-between">
              <div className="basis-[50%]">
                ...
                <p className="text-sm text-[#4A4A4A] font-bold">
                  {item.total_tickets - item.purchasedTicket === 0 ? (
                    <b>SOLD OUT</b>
                  ) : (
                    `${item.total_tickets - item.purchasedTicket}/${item.total_tickets}`
                  )}
                </p>
              </div>
              <div className="basis-[49%]">
                <p className="text-sm">Ticket Price</p>
                <p className="text-sm text-[#4A4A4A] font-bold">{item.price} $BAD</p>
              </div>
            </div>
            <div className="flex justify-between pt-2">
              ...
              <div className="basis-[49%]">
                ...
                <p className="text-sm text-[#4A4A4A] font-bold">{item.myTicket}</p>
              </div>
            </div>
            <div className="flex justify-between pt-2 pb-9">
              {Date.now() / 1000 < item.end_date && Date.now() / 1000 > item.start_date && (
                <div className="flex basis-[50%] justify-between items-start">
                  ...
                  <button
                    type="button"
                    className={`basis-[50%] text-sm bg-black rounded-md text-white py-1 border border-black  ${
                      buyTicketNFT.status ? `opacity-100 cursor-pointer` : `opacity-80 cursor-pointer `
                    } `}
      ...
      <div className="-mt-[26px] text-center">
        {currentTime > item.endTime || item.total_tickets - item.purchasedTicket === 0 ? (
          <Link
            to={`/raffle/${item._id}`}
