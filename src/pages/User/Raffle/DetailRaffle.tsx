...
const UserDetailRaffle = () => {
  ...
  const handleBuyTicket = async () => {
    try {
      ...
      if (res) {
        ...
        setBuyerInfo({
          ...buyerInfo,
          purchasedTicket: buyerInfo?.purchasedTicket ? buyerInfo?.purchasedTicket + _amount : _amount,
        });
        setNftInfo({
          ...nftInfo,
          purchasedTicket: nftInfo?.purchasedTicket ? nftInfo?.purchasedTicket + _amount : _amount,
        ...
        const finditem = ticketBuyerLists.find(
          (item: any) => item?.buyer?.toString() === anchorWallet?.publicKey?.toString()
        );
        ...
        if (finditem) {
          const new_buyerLists = ticketBuyerLists.map((item: any, idx: any) => {
            return idx === findIdx
              ? { ...ticketBuyerLists[findIdx], purchasedTicket: ticketBuyerLists[findIdx].purchasedTicket + _amount }
              : item;
          });
    ...
    if (get_user) {
      if (get_user?.twitterName) {
        const filter = filterBuyerLists.map((item) => {
          return item?.buyer?.toString() === get_user?.walletAddress ? { ...item, name: get_user?.twitterName } : item;
        });
        setTicketBuyerLists(filter);
      } else if (get_user?.discordName) {
        const filter = filterBuyerLists.map((item) => {
          return item?.buyer?.toString() === get_user?.walletAddress ? { ...item, name: get_user?.discordName } : item;
        });
        setTicketBuyerLists(filter);
      } else {
  ...
  const getRaffleStatus = async (poolData: any) => {
    ...
    if (Date.now() > poolData.endTime * 1000) {
      status = 2;
    } else if (Date.now() > poolData.startTime * 1000) {
      status = 1;
    }
    ...
    if (poolData?.state >= 1) {
      ...
      try {
        const claim_status = poolData?.buyers.find((item: any) => item?.isWinner === 1 && item?.claimedPrize);
        if (claim_status)
          setWinnerPrized({
            ...winnerPrized,
            prized: true,
            ...
            <div className="xl:basis-[35%] max-w-[450px] m-auto xl:max-w-auto xl:m-0 pb-6 xl:pb-0">
              ...
              <div className="mt-5 flex items-cetner justify-center">
                <button type="button" className="text-white bg-btngrad rounded-full flex items-center py-3 px-5">
                  <img src={PricetagIcon} alt="Pricetag-icon" className="w-[22px]" />
                  <span className="ml-3 text-lg">Price: {nftInfo.price} $BAD per Ticket</span>
                </button>
              </div>
              <div className="mt-5">
                {!anchorWallet && (
                    ...
                    {isLoading ? (
                      ...
                      <div className="text-center mt-5">
                        <div className="flex items-center justify-between">
                          <input
                            ...
                            className="w-[48%] block text-white text-base text-center outline-none bg-transparent border border-[#ffffff99] rounded-full py-3 px-5"
                            disabled={raffleStatus !== 1}
                          />
                          {raffleStatus !== 3 && raffleStatus !== 2 && !(raffleStatus === 1 && remainTickets === 0) && (
                            <button
                              type="button"
                            ...
                            >
                              <span className="sm:text-lg text-sm text-black">Buy Ticket(s)</span>
                            </button>
                          )}
                      ...
                      (raffleStatus === 1 && remainTickets === 0) &&  <p className="text-white text-[1.25rem]">Pending</p>
                        ...
                        {(raffleStatus === 2 || (raffleStatus === 1 && remainTickets === 0)) && (
                          <p className="text-white text-[1.25rem]">Waiting</p>
                        )}
                        {raffleStatus === 3 && (
                          <>
                            <div className="flex items-center justify-between mt-5">
                              ...
                              >
                                ...
                                <h1 style={{ fontSize: "26px", fontWeight: "bold" }}>
                                  ...
                                  {winnerWalletAddress !== "" &&
                                    winnerWalletAddress !== anchorWallet?.publicKey.toString() && (
                                      <Link to={`/profile/raffle/${winnerWalletAddress}`}>
                                ...
                                {winnerWalletAddress !== "" &&
                                  winnerWalletAddress === anchorWallet?.publicKey.toString() &&
                                  !winnerPrized.prized && (
                                    <div className="btn-gradient rounded-full p-[1px]">
                                      <div className="btn-background-absolute rounded-full">
                                        <p
                                          className={`text-[1.25rem] text-center py-2 px-8 block text-[#15BFFD] m-0 ${
                                            !winnerPrized.prized ? "cursor-pointer" : "cursor-no-drop"
                                          }`}
                                ...
                                {winnerWalletAddress !== "" && (
                                  <p className="text-[orange]">Won with {winnerTicket} ticket(s)</p>
                                )}
                              </div>
                            </div>
                          </>
                  ...
                  <div className="bg-[#00000031] py-4 px-4 sm:px-0 mt-4">
                    ...
                    <div className="sm:flex block justify-between sm:w-[85%] m-auto mt-5">
                      ...
                      <div className="text-center md:px-10 px-6 py-4 sm:py-0 my-4 sm:my-0 sm:border-r sm:border-l border-dashed">
                        ...
                        {anchorWallet ? (
                          <p className="text-white">{buyerInfo ? buyerInfo.purchasedTicket : 0}</p>
                        ) : (
                ...
                {raffleInfo === "participants" && (
                  <div className="text-white py-4 max-h-[447px] overflow-y-auto" id="wallet-list">
                    <ul className="py-3 px-4 w-full flex justify-between">
                      ...
                      <li className="basis-[50%] text-xl text-center">Tickets Bought</li>
                    </ul>
                        ...
                        <ul className="py-2 px-4 w-full flex justify-between">
                          <li className="basis-[50%] text-base">
                            {item?.name ? (
                              ...
                              <Link to={`/profile/raffle/${item?.buyer.toBase58()}`}>
                                {item?.name
                                  ? item?.name
                                  : item?.buyer.toBase58()?.substr(0, 6) +
                                    "..." +
                                    item?.buyer.toBase58()?.substr(item?.buyer.toBase58().length - 4, 4)}
                              </Link>
                            )}
