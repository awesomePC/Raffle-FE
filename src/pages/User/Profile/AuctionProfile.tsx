  ...
  const getData = async () => {
    try {
      ...
      for (let i = 0; i < getAuctions.length; i++) {
        ...
        const findMeINPoolData = poolData.bids.find(
          (item: any) =>
        ...
        let high = 0;
        if (winner_bidderLists.length > 0) high = Math.max.apply(Math, winner_bidderLists);

        const result: any = filterBidderLists.find((item: any) => item.price.toNumber() === high);

        const winner = poolData.bids.find(
          (item: any) => item.isWinner === 1 && item.bidder.toString() === walletAddress
        );
        ...
        if (findMeINPoolData) {
          ...
          if (winner) filter_item = { ...filter_item, winnerWalletAddress: walletAddress };
      ...
      let _unclaimedList = get_filterField.filter(
        (item, index) =>
          item.isClaimed === 0 && item.isWinner === 0 && currentTime > item.endTime && item.topBidder !== walletAddress
      );
  ...
  const handleConnectDiscord = async () => {
    try {
      ...
      const verifyToken: any = await createUser(
        ...
        signedMessage ? base58.encode(signedMessage!) : null
      );
    ...
    <>
      ...
      <div className="border-white border-b-2">
        <div className="flex items-center justify-between py-5 px-4">
          <h1 className="text-2xl text-white">Welcome BAD</h1>
          {anchorWallet?.publicKey.toString() === walletAddress && (
            <div className="flex">
              <button
                type="button"
      ...
      {isAllTab &&
        (participantLists.length > 0 ? (
          participantLists.map((item: any, idx: any) => <AuctionRarticipant item={item} idx={idx} key={idx} />)
        ) : isLoading ? (
          <></>
        ) : (
          <div className="max-w-[1280px] m-auto px-4">
            <div className="bg-white rounded-md py-8 px-8 flex items-center">
              ...
              <h1 className="xl:text-[3.2rem] lg:text-[2.5rem] md:text-[1.8rem] ml-10">
                {anchorWallet
                  ? anchorWallet?.publicKey.toString() === walletAddress
                    ? "You havn’t participated in any Auctions!"
                    : "This wallet hasn’t participated in any Auctions!"
