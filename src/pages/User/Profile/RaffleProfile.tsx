  ...
  const getData = async () => {
    try {
      ...
      for (let i = 0; i < getRaffle.length; i++) {
        ...
        const winner = poolData.buyers.find(
          (item: any) => item.isWinner === 1 && item.buyer.toString() === walletAddress
        );
        console.log("winner", winner);
        const findMeINPoolData = poolData.buyers.find(
          (item: any) =>
        ...
        <div className="max-w-[1280px] m-auto px-4">
          <div className="bg-white rounded-md py-8 px-8 flex items-center">
            ...
            <h1 className="xl:text-[3.2rem] lg:text-[2.5rem] md:text-[1.8rem] ml-10">
              {anchorWallet
                ? anchorWallet?.publicKey.toString() === walletAddress
                  ? "You havn’t participated in any Raffles!"
                  : "This wallet hasn’t participated in any Raffles!"
