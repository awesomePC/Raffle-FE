...
const RaffleRarticipant = (props: any) => {
  ...
  const startCountdownRenderer = ({ api, days, hours, minutes, seconds, completed }: any) => {
    ...
    return completed ? (
      <Countdown ref={setEndCountdownRef} date={item.endTime * 1000} zeroPadTime={3} renderer={endCountdownRenderer} />
    ) : (
      <div>
        <p>Starts In</p>
        <p>
  ...
  return (
    <div key={idx}>
      <div className="max-w-[1280px] m-auto px-4">
        <div
          className={` border-2 ${
            item.winnerWalletAddress ? "border-[orange]" : "border-[white]"
          } bg-[#606060A6] rounded-md mb-4
        `}
        >
          <div className="flex p-4">
                ...
                <div className="flex flex-col justify-between h-full">
                  <div>
                    <div className="flex items-center">
                      ...
                      <span className="text-lg text-white inline-block ml-1">{item?.tokenName}</span>
                    </div>
                    <h1 className="text-[20px] text-white mt-1">{item?.name}</h1>
                  </div>
                  <div>
                    <Link
                      to={`/raffle/${item?.id}`}
            ...
            <div className="basis-[70%] flex justify-around pt-2">
              <div className="text-center flex flex-col items-center">
                <img src={TimingIcon} alt="TimingIcon" className="mb-2 w-[60px]" />
