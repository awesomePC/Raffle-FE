...
const AdminAuction = () => {
  ...
  const getData = async () => {
    try {
      ...
      for (let i = 0; i < getAuction.length; i++) {
        ...
        if (exist_pool) {
          ...
          for (let i = 0; i < bids.length; i++) {
            winner_buyerLists.push(Number(bids[i].price()));
          }
          let high = 0;
          if (winner_buyerLists.length > 0) high = Math.max.apply(Math, winner_buyerLists);

          const result: any = bids.find((item: any) => item.isWinner === 1);
          console.log("result", bids);
          let winnerWalletAddress = "";
          if (result) winnerWalletAddress = result?.bidder?.toString();
      ...
      const pastData = final_auction_All.filter((item: any) => item.end_date < Date.now() / 1000);
    ...
    <div className="bg-black">
      ...
      <div className="max-w-[1280px] m-auto pt-8 px-4">
        <div className="min-h-[50vh] md:min-h-[65vh]">
          ...
          {isFilterAuction && (
            <>
              <h1 className="text-3xl mt-8 text-white font-cb">Filter auctions</h1>
              {isLoading ? (
                        ...
                        <div className="flex gap-[1rem] flex-wrap">
                          {isFilterByItem.length > 0 ? (
                            isFilterByItem.map((item: any, id: any) => (
                              <FilterAuction item={item} id={id} url={Backend_URL} />
                            ))
                          ) : (
          ...
          {isFeatured && (
            <>
              <h1 className="text-3xl mt-8 text-white font-cb">Featured Auctions</h1>
              ...
              <div className="flex gap-[1rem] flex-wrap pb-12 md:max-w-[768px] lg:max-w-[100%] sm:max-w-[100%] md:m-auto">
                {isLoading ? (
                  ...
                  featuredData.map((item: any, id: any) => {
                    return <AuctionItem item={item} url={Backend_URL} id={id} />;
                  })
                ) : (
                  <div className="text-white">There is no Auction Data</div>
                )}
          ...
          {isPastAuction && (
            <>
              <h1 className="text-3xl mt-8 text-white font-cb">Past Auction</h1>
              <div className="flex gap-[1rem] flex-wrap pb-12">
                {isLoading ? (
                  <div id="preloader"></div>
