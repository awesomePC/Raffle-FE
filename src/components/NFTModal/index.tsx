...
const NFTModal = (props: any) => {
  const { show, address, connection, title, setNftName, raffleValue, setRaffleValue, auctionValue, setAuctionValue } =
    props;
  ...
  const onOkBtn = async () => {
    if (raffleValue) {
      ...
      // setNftName(selectedNft?.name ? selectedNft?.name : ``)
    }
  ...
  return (
    <>
      {isModalLoading ? <div id="preloader"></div> : <div id="preloader" style={{ display: "none" }}></div>}
      {show ? (
        <div className="fixed w-full h-full top-0 left-0 custom-blur-bg flex items-center justify-center z-50 px-2 py-12 overflow-y-auto">
          <div className="w-full max-w-[920px] m-auto border md:px-8 px-4 md:py-8 py-4 border-pink bg-blackbg rounded-[0.7rem]">
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer basis-[22%] rounded-[18px] ${
                      isActive === index ? `border-[4px] border-[#85ff00] border-solid ` : ` border-solid `
                    } `}
                    ...
                    <div className={`  rounded-[0.9rem] overflow-hidden   transition duration-1000`}>
                      ...
                      <div className="bg-[#606060] pt-2 -mt-1">
                        <div className="flex items-center mb-1">
                          <img src={VerificationIcon} alt="VerificationIcon" style={{ width: "16px" }} />
                        ...
                        <div className="bg-[#949494] flex overflow-hidden">
                          <p className="bg-white text-sm  px-2 py-1 para-clip-create basis-[67%]">Token ID</p>
